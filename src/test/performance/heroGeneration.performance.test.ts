...
describe('Desempenho de geração de herói', () => {
  ...
  it('valida criação de herói em tempo adequado', () => {
    ...
    useHeroStore.setState({ heroes: [], selectedHeroId: null, availableQuests: [], guilds: [], referralInvites: [] } as any)
    const data = {
      name: 'Teste',
      race: 'humano',
      class: 'guerreiro',
  ...
  it('elementos ajustam acerto e efetividade (thunder e light vs dark)', () => {
    ...
    const rNeutral = resolveAttack(attackerBase, defender, { attackElement: 'physical', defendElement: 'dark' })
    spy.mockRestore()
    const spy2 = (globalThis as any).vi.spyOn(Math, 'random')
    spy2.mockImplementationOnce(() => 0.52)
    const rThunder = resolveAttack(attackerBase, defender, { attackElement: 'thunder', defendElement: 'dark' })
    spy2.mockRestore()
    expect(rNeutral.hit).toBe(false)
    expect(rThunder.hit).toBe(true)
  })

  it('elemento air aumenta críticos e alinhamento mal concede drenagem de vida no combate', () => {
    ...
    const r = resolveAttack(attackerCaoticoMal, defender, { attackElement: 'air', defendElement: 'physical' })
    spy.mockRestore()
    ...
    const hero: any = {
      ...
      attributes: { forca: 15, destreza: 1, constituicao: 6, inteligencia: 3, sabedoria: 2, carisma: 2 },
    ...
    const lifestealLine = res.log.find(l => l.includes('drena') || l.includes('rouba'))
    expect(lifestealLine).toBeTruthy()
  })
})