...
export async function listEvents(viewerId: string, opts: { tag?: string; ownerId?: string; limit?: number; offset?: number } = {}): Promise<SocialEvent[]> {
  ...
  const res = await fetch(`/api/events/list?${q.toString()}`);
...
export async function createEvent(payload: {
  ...
  const res = await fetch('/api/events/create', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
...
export async function getEvent(id: string, viewerId?: string): Promise<SocialEvent | null> {
  const q = new URLSearchParams();
  if (viewerId) q.set('viewerId', viewerId);
  const res = await fetch(`/api/events/${encodeURIComponent(id)}?${q.toString()}`);
...
export async function updateEvent(id: string, actorId: string, updates: Partial<Omit<SocialEvent, 'id' | 'ownerId' | 'createdAt'>>): Promise<SocialEvent> {
  const res = await fetch(`/api/events/${encodeURIComponent(id)}/update`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ actorId, updates }) });
...
export async function deleteEvent(id: string, actorId: string): Promise<boolean> {
  const res = await fetch(`/api/events/${encodeURIComponent(id)}/delete`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ actorId }) });
...
export async function attendEvent(id: string, viewerId: string, status: 'yes' | 'no' | 'maybe'): Promise<SocialEvent> {
  const res = await fetch(`/api/events/${encodeURIComponent(id)}/attend`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ viewerId, status }) });
  if (!res.ok) {
    if (res.status === 409) {
...
export async function fetchEventChat(id: string): Promise<EventMessage[]> {
  const res = await fetch(`/api/events/${encodeURIComponent(id)}/chat`);
...
export async function sendEventChat(id: string, viewerId: string, text: string): Promise<EventMessage> {
  const res = await fetch(`/api/events/${encodeURIComponent(id)}/chat`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ viewerId, text }) });
...
export async function addEventMedia(id: string, viewerId: string, url: string, caption?: string): Promise<EventMedia> {
  const res = await fetch(`/api/events/${encodeURIComponent(id)}/media`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ viewerId, url, caption }) });
...
export async function listEventMedia(id: string): Promise<EventMedia[]> {
  const res = await fetch(`/api/events/${encodeURIComponent(id)}/media`);
...
export async function rateEvent(id: string, viewerId: string, stars: number, comment?: string): Promise<boolean> {
  const res = await fetch(`/api/events/${encodeURIComponent(id)}/rate`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ viewerId, stars, comment }) });
...
export async function recommendEvents(viewerId: string, interestsCsv?: string, opts: { limit?: number; offset?: number } = {}): Promise<SocialEvent[]> {
  ...
  if (typeof opts.limit === 'number') q.set('limit', String(opts.limit));
  if (typeof opts.offset === 'number') q.set('offset', String(opts.offset));
  const res = await fetch(`/api/events/recommendations?${q.toString()}`);
