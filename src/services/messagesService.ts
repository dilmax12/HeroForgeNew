...
export async function sendSystemGuildLetter(toHeroId: string, toHeroName: string, subject: string, body: string): Promise<{ ok: boolean; error?: string; offline?: boolean; letter?: GuildLetter }>{
  const letter: GuildLetter = {
    id: `letter-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`,
    author_id: 'guild-system',
    author_name: 'Guilda',
    to_hero_id: toHeroId,
    to_hero_name: toHeroName,
    subject: subject.trim(),
