  ...
  return (
    <div className="bg-white p-4 rounded-lg border border-gray-200">
      <div className="flex items-center justify-between mb-2">
        <h3 className="text-lg font-semibold text-gray-800">👥 Party</h3>
      ...
      {party ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          {memberNames.map(m => (
            <div key={m.id} className="flex items-center justify-between p-2 rounded border bg-gray-50">
              <div className="font-medium text-gray-800 flex items-center gap-2">{m.name}{m.isNPC && <span className="text-xs bg-gray-200 text-gray-700 px-2 py-0.5 rounded">NPC</span>}</div>
              <div className="text-xs text-gray-500">ID: {m.id.slice(0,6)}</div>
            </div>
          ))}
        </div>
      ) : (
        <div className="text-gray-600">Você não está em nenhuma party.</div>
      )}
